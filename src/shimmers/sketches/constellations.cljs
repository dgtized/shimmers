...
(defn download []
  (let [comment (str "Generated By: "
                     (.-href (.-location js/window))
                     "\nAt: " (.toUTCString (js/Date.)) "\n")
        filename "constellations.svg"
        button (svg-export/download "scene" filename comment)]
    [:input {:type "button" :value "Download" :on-click button}]))
